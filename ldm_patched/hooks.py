...
from ldm_patched.contrib.node_helpers import conditioning_set_values
...
class TransformerOptionsHook(Hook):
    ...
    def add_hook_patches(self, model: ModelPatcher, model_options: dict, target_dict: dict[str], registered: HookGroup):
        ...
        registered.add(self)
        ldm_patched.modules.patcher_extension.merge_nested_dicts(model_options, add_model_options, copy_dict1=False)
        return True

    def on_apply_hooks(self, model: ModelPatcher, transformer_options: dict[str]):
        if not self._skip_adding:
            ldm_patched.modules.patcher_extension.merge_nested_dicts(transformer_options, self.transformers_dict, copy_dict1=False)
